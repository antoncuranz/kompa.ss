...
export function AmbiguousFlightDialogContent({
  ...
  const canConfirm = ambiguousFlightLegs.every(leg =>
    selectedChoices.get(leg.legIdx) !== undefined
  )
  ...
  return (
    <>
        ...
        {ambiguousFlightLegs.map(({legIdx, ...leg}) => {
          ...
          return (
            <div key={legIdx} className="space-y-3">
              <h3 className="font-semibold">Flight {leg.flightNumber}{legLabel}</h3>
              <p className="text-sm text-muted-foreground">Date: {formatDateShort(leg.date)}</p>
      ...
      <DialogFooter>
        <Button className="w-full text-base" onClick={handleConfirm} disabled={!canConfirm}>
          Confirm Selection
        </Button>
      </DialogFooter>
    </>
