...
export async function fetchTrip(tripId: number) {
  ...
  return {
    ...trip,
    startDate: parseLocalDateTime(trip.startDate),
    endDate: parseLocalDateTime(trip.endDate),
  }
}
...
export async function fetchAccommodation(tripId: number) {
  ...
  return accommodation.map(acc => ({
    ...acc,
    arrivalDate: parseLocalDateTime(acc.arrivalDate),
    departureDate: parseLocalDateTime(acc.departureDate)
  }))
}
...
export async function fetchFlights(tripId: number) {
  ...
  return flights.map(flight => ({
    ...
    legs: flight.legs.map(leg => ({
      ...leg,
      departureDateTime: parseLocalDateTime(leg.departureDateTime),
      arrivalDateTime: parseLocalDateTime(leg.arrivalDateTime)
    }))
  }))
}
...
function parseLocalDateTime(timestamp: Date|string): Date {
  // timestamp is actually a string, but don't tell typescript!
